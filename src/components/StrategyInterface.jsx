...
import { User } from 'firebase/auth'; // Not used in component body, but useful for type hinting
...
const StrategyInterface = ({ onViewEngine, db, auth, currentUser, appId, onSignOut, onOpenAuthModal }) => {
  ...
  // CRITICAL: Fetch the key from the environment variables exposed by Vercel/Vite
  const GEMINI_API_KEY = import.meta.env.VITE_GEMINI_API_KEY; 
  
  // Construct the full prompt for the Gemini API call (Remains the same)
  ...
  // Function to save the project data to Firestore (Remains the same)
  ...
  const handleNext = async (e) => {
    ...
    // Custom validation (Remains the same)
  ...
  // --- Render Logic ---
  const renderContent = () => {
      // ... (Rest of renderContent remains the same)
      if (isProcessing) {
          // 1. Processing State
          return (
              <motion.div
